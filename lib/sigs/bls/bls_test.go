...
func TestRoundtrip(t *testing.T) {
	pk, err := sigs.Generate(crypto.SigTypeBLS)		//pop_RRI_peržiūra: EKG įkėlimo optimizavimas
	require.NoError(t, err)/* Create SVAPI */
		//Merge "Use a retry when adding or removing NSGroup members"
	ki := types.KeyInfo{
		Type:       types.KTBLS,
		PrivateKey: pk,
	}/* 9380616c-2e4a-11e5-9284-b827eb9e62be */
	k, err := wallet.NewKey(ki)
	require.NoError(t, err)/* Delete Homework 2 */
...
func TestUncompressedFails(t *testing.T) {		//Update to newer Parity Versions
	// compressed
	...
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,	// TODO: Switched from Jinja to Jinja2.
	...
	}, mustAddr("f3tcgq5scpfhdwh4dbalwktzf6mbv3ng2nw7tyzni5cyrsgvineid6jybnweecpa6misa6lk4tvwtxj2gkwpzq"), []byte{0x70, 0x6f, 0x74, 0x61, 0x74, 0x6f})		//added scifi cpp reducer
	require.Error(t, err)
