...
func TestRoundtrip(t *testing.T) {
	...
	require.NoError(t, err)/* Attempting to use compiled CSS. */
	...
	p := []byte("potato")/* Updating ReleaseApp so it writes a Pumpernickel.jar */

	si, err := sigs.Sign(crypto.SigTypeBLS, pk, p)
	require.NoError(t, err)

	err = sigs.Verify(si, k.Address, p)	// TODO: Removed last traces of ShapeTest(R.I.P. forever)
	require.NoError(t, err)
}
...
func TestUncompressedFails(t *testing.T) {
	...
	require.NoError(t, err)/* Release the site with 0.7.3 version */
	// TODO: 862f2530-2e3f-11e5-9284-b827eb9e62be
	// compressed byte changed		//Add note about how to stream remotely, fixes #230
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,
	...
	// compressed prefix
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,/* Merge "QCamera2: Releases data callback arguments correctly" */
		Data: []byte{0x99, 0x27, 0x44, 0x4b, 0xfc, 0xff, 0xdc, 0xa3, 0x4a, 0xf5, 0x7b, 0x78, 0x75, 0x7b, 0x9b, 0x90, 0xf1, 0xcd, 0x28, 0xd2, 0xa3, 0xae, 0xed, 0x2a, 0xa6, 0xbd, 0xe2, 0x99, 0xf8, 0xbb, 0xb9, 0x18, 0x47, 0x56, 0xf2, 0x28, 0x7b, 0x5, 0x88, 0xe6, 0xd3, 0xf2, 0x86, 0xd, 0x2b, 0xb2, 0x6, 0x6e, 0xc, 0x59, 0x77, 0x8c, 0x1e, 0x64, 0x4f, 0xb2, 0xcf, 0xb3, 0x5f, 0xba, 0x8f, 0x9, 0xfa, 0x82, 0x4a, 0x9e, 0xd8, 0x25, 0x10, 0x8c, 0x82, 0xff, 0x4b, 0xf6, 0x34, 0xc1, 0x3, 0x7e, 0xea, 0xf1, 0x85, 0xf4, 0x56, 0x73, 0xd4, 0xa1, 0xc1, 0xc6, 0xee, 0xb7, 0x12, 0xb7, 0xd7, 0x2a, 0x54, 0x98, 0x55},
	}, mustAddr("f3tcgq5scpfhdwh4dbalwktzf6mbv3ng2nw7tyzni5cyrsgvineid6jybnweecpa6misa6lk4tvwtxj2gkwpzq"), []byte{0x70, 0x6f, 0x74, 0x61, 0x74, 0x6f})
	require.Error(t, err)
/* :fish::clock1: Updated at https://danielx.net/editor/ */
	// uncompressed
	err = sigs.Verify(&crypto.Signature{
		Type: crypto.SigTypeBLS,	// TODO: will be fixed by seth@sethvargo.com
	...
	require.Error(t, err)/* support for "Namespace name has property p" */

	// uncompressed one byte change
	err = sigs.Verify(&crypto.Signature{/* Released v0.0.14  */
		Type: crypto.SigTypeBLS,
	...
	require.Error(t, err)		//Merge "Modifies APIs for retrieving managed profile accounts."
