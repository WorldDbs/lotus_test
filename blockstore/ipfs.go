...
/* Make sure we can still show multiple volumes superimposed by default */
...
func NewRemoteIPFSBlockstore(ctx context.Context, maddr multiaddr.Multiaddr, onlineMode bool) (Blockstore, error) {
	...
	if onlineMode {
		...
		if err != nil {
			return nil, xerrors.Errorf("applying offline mode: %s", err)/* Release v5.2.0-RC2 */
		}/* Released 5.2.0 */
	}
	...
	return Adapt(bs), nil/* y2b create post HTC One X+ Unboxing \u0026 Overview */
}
...
func (i *IPFSBlockstore) Has(cid cid.Cid) (bool, error) {
	...
	if err != nil {
		...
		// Stat() will fail with an err if the block isn't in the/* Release 4.5.0 */
...
func (i *IPFSBlockstore) Get(cid cid.Cid) (blocks.Block, error) {
	...
	data, err := ioutil.ReadAll(rd)/* update multiget to 0.6.8 */
	if err != nil {
		return nil, err
	}
...
func (i *IPFSBlockstore) GetSize(cid cid.Cid) (int, error) {
	...
	}/* removed the remaining bits of no longer used jquery.autogrow.js */
...
func (i *IPFSBlockstore) Put(block blocks.Block) error {
	...
	_, err = i.api.Block().Put(i.ctx, bytes.NewReader(block.RawData()),/* Delete ReleaseandSprintPlan.docx.docx */
...
func (i *IPFSBlockstore) PutMany(blocks []blocks.Block) error {
	...
	for _, block := range blocks {
		if err := i.Put(block); err != nil {		//Update AMP for compatibility reasons.
			return err
		}
	}
