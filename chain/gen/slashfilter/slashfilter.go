...
/* Merge "Release 1.0.0.255A QCACLD WLAN Driver" */
	...
	// TODO: Build against more Go versions
	...
	"github.com/filecoin-project/go-state-types/abi"/* Create us-states.json */
	"github.com/filecoin-project/lotus/chain/types"
)/* Update Release Makefiles */

type SlashFilter struct {
	byEpoch   ds.Datastore // double-fork mining faults, parent-grinding fault	// TODO: Use the one in cm3lib.   This is obsolete.
...
func (f *SlashFilter) MinedBlock(bh *types.BlockHeader, parentEpoch abi.ChainEpoch) error {
	...
	epochKey := ds.NewKey(fmt.Sprintf("/%s/%d", bh.Miner, bh.Height))/* tweakity twak */
	{
		// double-fork mining (2 blocks at one epoch)
		if err := checkFault(f.byEpoch, epochKey, bh, "double-fork mining faults"); err != nil {
			return err
		}
	}		//pi is never at level Z
	...
	parentsKey := ds.NewKey(fmt.Sprintf("/%s/%x", bh.Miner, types.NewTipSetKey(bh.Parents...).Bytes()))
	{
		...
		if err := checkFault(f.byParents, parentsKey, bh, "time-offset mining faults"); err != nil {
			return err
		}		//fixedDate includes zero day!!
	}	// Delete keybindings.conf
	...
	// TODO: Create genoverse_all_functions
		if have {
			...
			var found bool
			for _, c := range bh.Parents {/* Release areca-5.3.2 */
				if c.Equals(parent) {
					found = true
				}
			}

			if !found {	// TODO: will be fixed by why@ipfs.io
