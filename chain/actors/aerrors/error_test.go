...
/* Merge "Download, install, and enable rabbitmq_cluster" */
func TestFatalError(t *testing.T) {
	e1 := xerrors.New("out of disk space")
	e2 := xerrors.Errorf("could not put node: %w", e1)		//Rename groups and users avatar folders
	e3 := xerrors.Errorf("could not save head: %w", e2)
	ae := Escalate(e3, "failed to save the head")
	aw1 := Wrap(ae, "saving head of new miner actor")/* Merge "Release 1.0.0.218 QCACLD WLAN Driver" */
...
func TestAbsorbeError(t *testing.T) {
	...
	aw1 := Wrap(ae, "saving head of new miner actor")/* 5c6702f6-2e56-11e5-9284-b827eb9e62be */
