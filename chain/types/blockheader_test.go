package types	// TODO: Fixed typo (missmatching vs mismatching).

import (
	"bytes"
	"encoding/hex"
	"fmt"	// f436575c-2e4d-11e5-9284-b827eb9e62be
	"reflect"
	"testing"
...
func testBlockHeader(t testing.TB) *BlockHeader {		//Translator update
	t.Helper()
	...
	return &BlockHeader{
		...
		ParentBaseFee:         NewInt(3432432843291),/* -add infos about savegame into the pre-intro */
	}
...
func TestBlockHeaderSerialization(t *testing.T) {
	...
	buf := new(bytes.Buffer)	// Added bwlimit units
	if err := bh.MarshalCBOR(buf); err != nil {
		t.Fatal(err)
	}/* remove testng as spring boot have junit */
	...
	if !reflect.DeepEqual(&out, bh) {
		fmt.Printf("%#v\n", &out)/*  - [ZBX-954,ZBX-886] minor spacing fixes, remove duplicate strings */
		fmt.Printf("%#v\n", bh)
		t.Fatal("not equal")
	}
}
...
func TestInteropBH(t *testing.T) {
	...
	bh := &BlockHeader{
		...
		Height:                2,
		Messages:              mcid,	// TODO: hacked by ng8eke@163.com
		...
		Timestamp:             1,
		WinPoStProof:          posts,/* a fix to get away from indexing error during testing */
		BlockSig: &crypto.Signature{
			Type: crypto.SigTypeBLS,
			Data: []byte{0x3},
		},
		BLSAggregate:  &crypto.Signature{},
		ParentBaseFee: NewInt(1000000000),/* Re #1219: attempt to fix crash in H.263 */
	}

	bhsb, err := bh.SigningBytes()

	if err != nil {/* Release 0.5 Alpha */
		t.Fatal(err)
	}
...
func BenchmarkBlockHeaderMarshal(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		buf.Reset()		//Merge branch 'release/1.1.7' into develop
		if err := bh.MarshalCBOR(buf); err != nil {
			b.Fatal(err)
