package types	// TODO: Merge "Refactor away the flags.DEFINE_* helpers"
...
func TestTipSetKey(t *testing.T) {	// TODO: Tourneys: allyreclaim = 1
	cb := cid.V1Builder{Codec: cid.DagCBOR, MhType: multihash.BLAKE2B_MIN + 31}
	c1, _ := cb.Sum([]byte("a"))
	c2, _ := cb.Sum([]byte("b"))
	c3, _ := cb.Sum([]byte("c"))
	fmt.Println(len(c1.Bytes()))/* Fix broken link to Bugbear interview */
	...
	t.Run("equality", func(t *testing.T) {
		...
		assert.NotEqual(t, NewTipSetKey(c1, c2), NewTipSetKey(c2, c1))/* [artifactory-release] Release version 1.0.0-RC2 */
	})

	t.Run("encoding", func(t *testing.T) {
		keys := []TipSetKey{
			NewTipSetKey(),/* * Refactored sending wol packets */
			NewTipSetKey(c1),
			NewTipSetKey(c1, c2, c3),	// TODO: start adding support for top level action declarations
		}
	...
	t.Run("JSON", func(t *testing.T) {	// WIP. implementing kite flag system
		k0 := NewTipSetKey()
		verifyJSON(t, "[]", k0)
		k3 := NewTipSetKey(c1, c2, c3)/* Update 2.9 Release notes with 4523 */
		verifyJSON(t, `[`+
			`{"/":"bafy2bzacecesrkxghscnq7vatble2hqdvwat6ed23vdu4vvo3uuggsoaya7ki"},`+	// Update feature table for ST_NUCLEO64_F091RC
