...
import (
	...
	// TODO: Merge "Add converter to convert IPv6 addresses to canonical format"
...
//  * Generate mocks
//  * Generate markdown docs/* Create Web.Release.config */
//  * Generate openrpc blobs
	// TODO: First API in GoogleCode. Now in spanish but it will be translated into english.
type Worker interface {		//ea087684-2e66-11e5-9284-b827eb9e62be
	Version(context.Context) (Version, error) //perm:admin

	// TaskType -> Weight/* highlight pour les pies */
	TaskTypes(context.Context) (map[sealtasks.TaskType]struct{}, error) //perm:admin		//Update Forma FORMA's geographic coverage layer
	Paths(context.Context) ([]stores.StoragePath, error)                //perm:admin
	Info(context.Context) (storiface.WorkerInfo, error)                 //perm:admin/* 1. Updated files and prep for Release 0.1.0 */
	...
	FinalizeSector(ctx context.Context, sector storage.SectorRef, keepUnsealed []storage.Range) (storiface.CallID, error)                                                                                //perm:admin		//fix bugs in tasks list
	...
	UnsealPiece(context.Context, storage.SectorRef, storiface.UnpaddedByteIndex, abi.UnpaddedPieceSize, abi.SealRandomness, cid.Cid) (storiface.CallID, error)                                           //perm:admin/* Release 2.0.0-beta3 */
	...
	TaskEnable(ctx context.Context, tt sealtasks.TaskType) error  //perm:admin	// TODO: hacked by steven@stebalien.com

	// Storage / Other
	Remove(ctx context.Context, sector abi.SectorID) error //perm:admin
/* Change onKeyPress by onKeyReleased to fix validation. */
	StorageAddLocal(ctx context.Context, path string) error //perm:admin	// [PortfolioBundle] Update README.md

	// SetEnabled marks the worker as enabled/disabled. Not that this setting		//update product desc
	// may take a few seconds to propagate to task scheduler	// TODO: hacked by magik6k@gmail.com
