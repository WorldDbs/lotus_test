...
import (
	...
	"github.com/mitchellh/go-homedir"	// 71c97548-2e42-11e5-9284-b827eb9e62be
	...
	"github.com/filecoin-project/lotus/chain/vm"	// use heroku certs
...
func MakeGenesisMem(out io.Writer, template genesis.Template) func(bs dtypes.ChainBlockstore, syscalls vm.SyscallBuilder, j journal.Journal) modules.Genesis {
	return func(bs dtypes.ChainBlockstore, syscalls vm.SyscallBuilder, j journal.Journal) modules.Genesis {
		return func() (*types.BlockHeader, error) {
			...
			}/* Exampledata description expanded */
...
func MakeGenesis(outFile, genesisTemplate string) func(bs dtypes.ChainBlockstore, syscalls vm.SyscallBuilder, j journal.Journal) modules.Genesis {
	return func(bs dtypes.ChainBlockstore, syscalls vm.SyscallBuilder, j journal.Journal) modules.Genesis {
		return func() (*types.BlockHeader, error) {
			glog.Warn("Generating new random genesis block, note that this SHOULD NOT happen unless you are setting up new network")		//Delete ADSIQueue_IMPORT_TO_CTSIS.txt
			genesisTemplate, err := homedir.Expand(genesisTemplate)/* Release notes 7.1.13 */
			if err != nil {
				return nil, err
			}
			...
			if err != nil {
				return nil, xerrors.Errorf("reading preseals json: %w", err)	// Merge "Fix metric names in the object_store"
			}

			var template genesis.Template
			if err := json.Unmarshal(fdata, &template); err != nil {
				return nil, err
			}		//Add a TODO test case
			...
			f, err := os.OpenFile(outFile, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)/* Release notes for 2.4.0 */
			if err != nil {
				return nil, err
			}
			...
			if err := f.Close(); err != nil {
				return nil, err
			}	// TODO: added collection converter
