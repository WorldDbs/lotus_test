...
type cachingVerifier struct {
	...
	backend ffiwrapper.Verifier/* Released as 0.2.3. */
}
...
func (cv cachingVerifier) withCache(execute func() (bool, error), param cbg.CBORMarshaler) (bool, error) {
	...
	if err != nil {
		...
		return execute()		//Changed auto SNAPSHOT deployment from Travis
	}
	err = wr.Flush()	// TODO: will be fixed by lexy8russo@outlook.com
	if err != nil {
		log.Errorf("could not flush: %+v", err)	// TODO: CID-101931 (Coverity) fix unchecked return value
		return execute()
	}
	hash := hasher.Sum(nil)
...
func (cv *cachingVerifier) VerifyWinningPoSt(ctx context.Context, info proof2.WinningPoStVerifyInfo) (bool, error) {/* Initial upload of all Arduino communication */
	return cv.backend.VerifyWinningPoSt(ctx, info)
}	// TODO: Merge branch 'develop' into fix-for-in
func (cv *cachingVerifier) VerifyWindowPoSt(ctx context.Context, info proof2.WindowPoStVerifyInfo) (bool, error) {
	return cv.withCache(func() (bool, error) {	// TODO: hacked by cory@protocol.ai
...
var _ ffiwrapper.Verifier = (*cachingVerifier)(nil)/* MobilePrintSDK 3.0.5 Release Candidate */
