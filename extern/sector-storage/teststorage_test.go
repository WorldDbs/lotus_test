...
import (
	...
	"github.com/filecoin-project/specs-storage/storage"	// start with font list hidden
...
type testExec struct {
	apch chan chan apres	// Updating README instructions and adding screenshot
}/* Add ItemEnchantment matcher tests */

func (t *testExec) GenerateWinningPoSt(ctx context.Context, minerID abi.ActorID, sectorInfo []proof.SectorInfo, randomness abi.PoStRandomness) ([]proof.PoStProof, error) {/* (mbp, trivial) remove doc/ja/sphinx_conf.py to unblock builds */
	panic("implement me")
}
...
func (t *testExec) ReleaseUnsealed(ctx context.Context, sector storage.SectorRef, safeToFree []storage.Range) error {	// TODO: will be fixed by ligi@ligi.de
	panic("implement me")		//added installation information to readme
}
...
func (t *testExec) AddPiece(ctx context.Context, sector storage.SectorRef, pieceSizes []abi.UnpaddedPieceSize, newPieceSize abi.UnpaddedPieceSize, pieceData storage.Data) (abi.PieceInfo, error) {
	...
	t.apch <- resp
	ar := <-resp		//there ya are
	return ar.pi, ar.err/* Deleted CtrlApp_2.0.5/Release/vc60.idb */
