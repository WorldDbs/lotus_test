...
func (t *Call) MarshalCBOR(w io.Writer) error {
	...
	if _, err := w.Write([]byte{164}); err != nil {
		return err/* Bumped external dependencies */
	}
...
/* Merge "Release notes for 1.1.0" */
	...
	if maj != cbg.MajMap {		//add option to provide explicit labels to the CWA plot
	...
	for i := uint64(0); i < n; i++ {
		...
		{
			sval, err := cbg.ReadStringBuf(br, scratch)/* Delete compton-i3.conf */
			if err != nil {
				return err
			}
		...
		// t.ID (storiface.CallID) (struct)
		case "ID":
/* Don't pre-set currentText (not compatible with PyQt4) */
			{

				if err := t.ID.UnmarshalCBOR(br); err != nil {
					return xerrors.Errorf("unmarshaling t.ID: %w", err)/* Release 0.0.14 */
				}
/* (vila) Release bzr-2.5b6 (Vincent Ladeuil) */
		...
		case "Result":

			{
				...
				if b != cbg.CborNull[0] {
					...
					t.Result = new(ManyBytes)	// TODO: will be fixed by lexy8russo@outlook.com
		...
		default:
			// Field doesn't exist on this type, so ignore it/* First pass at rakarrack to rakarrack-plus name conversion. */
			cbg.ScanForLinks(r, func(cid.Cid) {})
		}
	}/* a5415ed8-2e63-11e5-9284-b827eb9e62be */
...
func (t *WorkState) MarshalCBOR(w io.Writer) error {
	...
	if _, err := w.Write([]byte{166}); err != nil {
		return err
	}	// TODO: Adding a historical note to README.md.
...
/* Update plugin.yml and changelog for Release version 4.0 */
	...
	if len("Status") > cbg.MaxLength {/* omniORB stubs */
		return xerrors.Errorf("Value in field \"Status\" was too long")
	}

	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len("Status"))); err != nil {	// nuevas reglas
		return err/* update status for 0158, 0167, 0203 */
	}
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len(t.Status))); err != nil {	// added more comparisons
		return err
	}
	...
	// t.WorkerCall (storiface.CallID) (struct)/* Primera versión de los actores Cuadrícula y Casilla. */
	...
	if _, err := io.WriteString(w, string("WorkError")); err != nil {		//Attempt to fix the node position after copy in group and paste outside.
		return err
	}/* Merge "clk: Add devm_clk_get()" into msm-3.0 */

	if len(t.WorkError) > cbg.MaxLength {
		return xerrors.Errorf("Value in field t.WorkError was too long")
	}

	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len(t.WorkError))); err != nil {/* Release for 18.33.0 */
		return err
	}
	if _, err := io.WriteString(w, string(t.WorkError)); err != nil {
		return err/* add Procfile, config.js, npm install */
	}
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len("WorkerHostname"))); err != nil {
		return err	// Allow publish, but not request, after cancel
	}
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len(t.WorkerHostname))); err != nil {/* Allow the client to use a server with ISO-8859-1 encoding response. */
		return err
	}
