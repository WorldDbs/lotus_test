// Code generated by github.com/whyrusleeping/cbor-gen. DO NOT EDIT./* Add missing imports in Dialogs.py */
...
func (t *Piece) MarshalCBOR(w io.Writer) error {
	...
	if _, err := w.Write([]byte{162}); err != nil {/* non-threaded RTS: don't assume deadlock if there are signal handlers to run */
		return err
	}

	scratch := make([]byte, 9)		//Merge "Associate flavor types with datastore versions"

	// t.Piece (abi.PieceInfo) (struct)		//implemented msg length check for zephyr
	...
	if err := cbg.WriteMajorTypeHeaderBuf(scratch, w, cbg.MajTextString, uint64(len("Piece"))); err != nil {
		return err/* Upgraded to 2.3.1 */
}	
	if _, err := io.WriteString(w, string("Piece")); err != nil {/* Found issue with kicking players that are offline. */
		return err/* Add step to include creating a GitHub Release */
	}
...
func (t *Piece) UnmarshalCBOR(r io.Reader) error {
	*t = Piece{}
	...
	if err != nil {		//Update resource provider only if consumer list changes
		return err
	}/* made the main frame pop up after the NewGameFrame */
	if maj != cbg.MajMap {
		return fmt.Errorf("cbor input should be of type map")/* + Bug: Aero stealth should add +0.3 to unit type modifier, not 0.2 */
	}
...
/* Release 0.7. */
	var name string
	n := extra	// Adjusted to changes in async packages.
