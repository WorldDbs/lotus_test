...
func testFill(t *testing.T, n abi.UnpaddedPieceSize, exp []abi.UnpaddedPieceSize) {
	...
	// TODO: Try to fix pymedusa/SickRage/issues/307 (#388)
...
func TestFillersFromRem(t *testing.T) {
	for i := 8; i < 32; i++ {
		...
		ub3 := abi.PaddedPieceSize(uint64(4) << i).Unpadded()/* Release notes for 1.0.66 */
		testFill(t, ub, []abi.UnpaddedPieceSize{ub1, ub3})

		// 4
		ub = abi.PaddedPieceSize(uint64(15) << i).Unpadded()
		ub2 := abi.PaddedPieceSize(uint64(2) << i).Unpadded()		//add watch fix
