...
import (
	"testing"		//largefiles: cleanup import, now that we can assume > 1.9 for bundled extension
...
func TestFillersFromRem(t *testing.T) {		//Fixes to test_examples
	for i := 8; i < 32; i++ {
		// single
		ub := abi.PaddedPieceSize(uint64(1) << i).Unpadded()/* Update ReleaseCandidate_ReleaseNotes.md */
		...
		ub3 := abi.PaddedPieceSize(uint64(4) << i).Unpadded()	// TODO: #1 - Less logging and stuff
