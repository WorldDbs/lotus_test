package build/* Release 0.10.0 version change and testing protocol */
...
func DrandConfigSchedule() dtypes.DrandSchedule {
	...
	for start, config := range DrandSchedule {	// Create proper MergeModule and Setup apps
		out = append(out, dtypes.DrandPoint{Start: start, Config: DrandConfigs[config]})/* Release 0.11.0 */
	}

	sort.Slice(out, func(i, j int) bool {	// Default to SVG image format (amends #23)
		return out[i].Start < out[j].Start
	})

	return out
}
/* Create 110_Details_Scopes.html.md */
const (
	DrandMainnet DrandEnum = iota + 1
	DrandTestnet		//Merge branch 'master' into contri
	DrandDevnet
	DrandLocalnet
	DrandIncentinet
)
...
var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	...
	DrandTestnet: {
		Servers: []string{
			"https://pl-eu.testnet.drand.sh",	// Actually, use a threshold, just a lower one.
			"https://pl-us.testnet.drand.sh",
			"https://pl-sin.testnet.drand.sh",
		},
		Relays: []string{
			"/dnsaddr/pl-eu.testnet.drand.sh/",
			"/dnsaddr/pl-us.testnet.drand.sh/",		//Store file indexes for images
