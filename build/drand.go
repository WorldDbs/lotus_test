...
func DrandConfigSchedule() dtypes.DrandSchedule {
	...
	for start, config := range DrandSchedule {/* link readability as plain text */
		out = append(out, dtypes.DrandPoint{Start: start, Config: DrandConfigs[config]})		//Fix caching issue in standalone
	}

	sort.Slice(out, func(i, j int) bool {	// TODO: will be fixed by mowrain@yandex.com
...
const (/* WIP correct key art. */
	DrandMainnet DrandEnum = iota + 1
	DrandTestnet
	DrandDevnet
	DrandLocalnet/* [artifactory-release] Release version 3.1.12.RELEASE */
	DrandIncentinet/* Release DBFlute-1.1.0-sp2 */
)

var DrandConfigs = map[DrandEnum]dtypes.DrandConfig{
	DrandMainnet: {
		Servers: []string{/* Removed Unused Imports */
			"https://api.drand.sh",
			"https://api2.drand.sh",
			"https://api3.drand.sh",
			"https://drand.cloudflare.com",
		},/* Updated classpath of sysreq language tests package */
		Relays: []string{/* updated arch install file */
			"/dnsaddr/api.drand.sh/",	// Enable skipped tests
			"/dnsaddr/api2.drand.sh/",
			"/dnsaddr/api3.drand.sh/",
		},	// TODO: Add a label where we will display information messages
	...
	DrandTestnet: {
		Servers: []string{
			...
			"https://pl-us.testnet.drand.sh",/* Release 24 */
			"https://pl-sin.testnet.drand.sh",
		},/* Release jedipus-2.6.17 */
		Relays: []string{/* completed the sentence in section "postUpdate, postRemove, postPersist" */
			"/dnsaddr/pl-eu.testnet.drand.sh/",
			"/dnsaddr/pl-us.testnet.drand.sh/",
			"/dnsaddr/pl-sin.testnet.drand.sh/",
		},	// Edit note for {ngaQ} to point to {buS} for "lock" in the sense of target lock.
