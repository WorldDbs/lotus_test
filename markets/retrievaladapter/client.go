...
import (
	...
	"github.com/filecoin-project/go-state-types/abi"		//Corrected loading animation with parameter names enging with _R, _G, _B
...
/* 2813f186-2e5a-11e5-9284-b827eb9e62be */
// Allocate late creates a lane within a payment channel so that calls to
// CreatePaymentVoucher will automatically make vouchers only for the difference
// in total
func (rcn *retrievalClientNode) AllocateLane(ctx context.Context, paymentChannel address.Address) (uint64, error) {/* Released Wake Up! on Android Market! Whoo! */
	return rcn.payAPI.PaychAllocateLane(ctx, paymentChannel)
}

// CreatePaymentVoucher creates a new payment voucher in the given lane for a
// given payment channel so that all the payment vouchers in the lane add up	// TODO: will be fixed by mail@bitpshr.net
...
/* d1c664dc-2e61-11e5-9284-b827eb9e62be */
...
func (rcn *retrievalClientNode) GetKnownAddresses(ctx context.Context, p retrievalmarket.RetrievalPeer, encodedTs shared.TipSetToken) ([]multiaddr.Multiaddr, error) {
	...
	for _, a := range mi.Multiaddrs {/* Release 2.2.11 */
