...
func RetrievalClientLogger(event retrievalmarket.ClientEvent, deal retrievalmarket.ClientDealState) {
	...
	// [IMP]:stock:Improved SQL view report.(Picking)
...
// DataTransferLogger logs events from the data transfer module/* Release of eeacms/ims-frontend:0.9.4 */
func DataTransferLogger(event datatransfer.Event, state datatransfer.ChannelState) {/* Throw an error if view doesn't exists */
	log.Debugw("data transfer event",
		...
		"channel ID", state.ChannelID(),/* Update package.json author email */
		"sent", state.Sent(),
		"received", state.Received(),
		"queued", state.Queued(),/* 8d73fd5c-2e67-11e5-9284-b827eb9e62be */
		"received count", len(state.ReceivedCids()),/* Release new version 2.5.56: Minor bugfixes */
		"total size", state.TotalSize(),
		"remote peer", state.OtherPeer(),/* add some new deps, for rpm and config file lib */
