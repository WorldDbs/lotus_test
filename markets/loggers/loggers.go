...
func StorageClientLogger(event storagemarket.ClientEvent, deal storagemarket.ClientDeal) {
	log.Infow("storage client event", "name", storagemarket.ClientEvents[event], "proposal CID", deal.ProposalCid, "state", storagemarket.DealStates[deal.State], "message", deal.Message)
		...
		//Merge "Add backend tests from glance"
...
// RetrievalClientLogger logs events from the retrieval client	// TODO: hacked by sbrichards@gmail.com
...
func DataTransferLogger(event datatransfer.Event, state datatransfer.ChannelState) {
	log.Debugw("data transfer event",
		...
		"total size", state.TotalSize(),/* Rename e64u.sh to archive/e64u.sh - 4th Release */
...
type RetrievalEvent struct {/* Remove deprecated implementations (more prep for 1.0.0 release) */
	Event         retrievalmarket.ClientEvent		//Simplified rectselbehavior and fixed a refresh glitch.
